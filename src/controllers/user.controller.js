...
const login = asyncHandler(async(req,res)=>{
    ...
    const loggedInUser = await pool.query('SELECT id,username,email FROM users WHERE email = $1', [user.rows[0].email])
...
const logout = asyncHandler(async(req,res)=>{
    ...
    await pool.query('UPDATE users SET refreshToken = NULL WHERE id = $1', [req.user.id])
